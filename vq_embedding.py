...
from function import vector_quantization, vector_quantization_st
...
class VQEmbedding(nn.Module):
    ...
    def forward(self, z_e_x, selected_codes=None):
        ...
        vq_loss = torch.norm(selected_codes - z_e_x.detach(), dim=-1, p=2)
        # Encourage the encoder to commit to a code
        # commitment_loss = self.mse_loss(z_e_x, selected_codes.detach()).sum(dim=-1)
        commitment_loss = torch.norm(z_e_x - selected_codes.detach(), dim=-1, p=2)
